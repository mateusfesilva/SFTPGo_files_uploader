...
def create_dir(remote_path, global_token):
    ...
    if resp.status_code not in [200, 201, 202]:
        raise Exception(f"Failed to create directory {remote_path}. Status code: {resp.status_code}")
    ...
    for i in range(3):
        try:
            ...
            if rp.status_code in [200, 201, 202]:
                content = rp.json()
                file_list = {item.get("name"): item.get("size") for item in content.get("name")}
                return file_list
        except Exception:
            time.sleep(5)
    return None
    ...
    for attempt in range(max_retries):
        try:
            with open(local_data, 'rb') as f:
                ...
                referer_url = f"{BASE_URL}?path={quote_plus(remote_path)}&_={time_stamp}"
                local_headers = {
                    "Accept": "*/*",
